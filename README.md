...
| File | Description |
|------|-------------|
| **`openapi.yaml`** | OpenAPI 3.0 spec — standardized docs for Swagger UI, codegen, tooling. All endpoints verified against live API |
...
HAR capture (browser)  →  analyze_har.py  →  HAR_ANALYSIS_FINDINGS.md
                                                   ↓
probe_new_endpoints.py  →  new_endpoints_responses.json  →  API.md (manual/docs update)
                                                   ↓
...
| Script | Purpose |
|--------|---------|
| **`verify_openapi.py`** | Validate `openapi.yaml` against live API — verifies all request examples and response schemas |
